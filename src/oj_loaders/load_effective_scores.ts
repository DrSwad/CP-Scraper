...
const compare_scores_ascending = (score1 : Score, score2 : Score) => {
...
async function load_effective_scores(
  contest_details: Array<Contest>,
  contestant_ids: Array<string>,
  contestant_scores: { [id: string]: Score },
  single_contest_scores: { [contest_id: string]: { [contestant_id: string]: Score }}) {
  const effective_scores: { [contestant_id: string]: Score } = {};

  contestant_ids.forEach(id => {
    let online_sorted_scores : Array<Score> = [];
    let onsite_sorted_scores : Array<Score> = [];
    let combined_sorted_scores : Array<Score> = [];
    ...
    onsite_sorted_scores.sort((score1, score2) => {
      if (compare_scores_ascending(score1, score2)) return -1;
      else if (compare_scores_ascending(score2, score1)) return 1;
      else return 0;
    });
