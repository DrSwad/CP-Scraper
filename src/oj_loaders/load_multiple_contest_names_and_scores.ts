...
async function load_multiple_contest_names_and_scores(contest_details: Array<Contest>, contestant_ids: Array<string>, score_overwrites: Array<ScoreOverwrite>, contestant_details: { [id: string]: Contestant }) {
  const contest_names: { [contest_id: string]: string } = {};
  const single_contest_scores :
    {
      [contest_id: string]: {
        [contestant_id: string]: Score
      };
    }
  = {};
  const combined_contest_scores: { [contestant_id: string]: Score } = {};
  ...
  score_overwrites.forEach(score_overwrite => {
    ...
    if (!oj_score_overwrites[score_overwrite.oj][score_overwrite.contest_id]) oj_score_overwrites[score_overwrite.oj][score_overwrite.contest_id] = {};
  ...
  for (const contest_detail of contest_details) {
    ...
    if (oj === "CF") load_single_contest_scores = load_single_cf_contest_name_and_scores(contest_id, contestant_ids, contestant_details);
    else if (oj === "AC") load_single_contest_scores = load_single_ac_contest_name_and_scores(contest_id, contestant_ids, contestant_details);
    else if (oj === "VJ") load_single_contest_scores = load_single_vj_contest_name_and_scores(contest_id, contestant_ids, contestant_details);
    else {
